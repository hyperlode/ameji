...
def create_ameji_from_emoji_files(output_folder, emoji_folder, unicode_to_tags, disregard_modifiers=True):
    ...
    '''
    Link the metadata to actual emoji. With the unicode code points.
    ...
    print("found emoji files in folder {} to create metadata for: {}".format(
        emoji_folder,
        len(files)
        ))
...
def create_metadata():
    ...
    BASE_PATH = r"E:\GIT\ameji\Resources\ameji metadata\emoji"
    
    # actual emoji, their file names are their unicode point    
    EMOJI_FOR_CODES_PATH = r"E:\GIT\ameji\Resources\symbols_svg\openmoji\openmoji-618x618-black"
    
    # metadata (tags and descriptions)
    UMMOJI_METADATA_PATH  = Path(BASE_PATH, r"ummoji\emoji_metadata_ummoji-client.txt")
    PATH_MUAN_EMOJILIB_TAGS = Path(BASE_PATH, r"muan_emojilib\emoji-en-US.json")
    ...
    PATH_METADATA_OUTPUT = Path(BASE_PATH,"test")
